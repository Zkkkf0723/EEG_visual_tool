...
# 初始化session_state中的关键变量
if 'data_loaded' not in st.session_state:
    st.session_state.data_loaded = False
if 'edf_path' not in st.session_state:
    st.session_state.edf_path = None
...
if run_button:
    ...
    with st.spinner("正在加载数据..."):
        try:
            # 清理旧的临时文件
            if st.session_state.edf_path and os.path.exists(st.session_state.edf_path):
                try:
            ...
            edf_path = os.path.join(temp_dir, f"temp_edf_{hash(edf_file.name)}.edf")
            with open(edf_path, "wb") as f:
                f.write(edf_file.getvalue())
            
            prob_path = None
            if prob_file is not None:
                prob_path = os.path.join(temp_dir, f"temp_prob_{hash(prob_file.name)}.pkl")
...
# 如果数据已加载，执行分析
if st.session_state.data_loaded and st.session_state.edf_path:
    try:
        ...
        enable_zscore = st.session_state.enable_zscore
        selected_age_group = st.session_state.selected_age_group
        window_sizes = st.session_state.window_sizes
        zscore_threshold = st.session_state.zscore_threshold
        prob_path = st.session_state.prob_path
        
        # 检查EDF文件是否存在
        if not os.path.exists(edf_path):
            st.error(f"❌ EDF文件不存在，请重新上传")
            st.session_state.data_loaded = False
            st.stop()
        
        with st.spinner("正在加载EDF数据..."):
...
def cleanup_temp_files():
    if st.session_state.edf_path and os.path.exists(st.session_state.edf_path):
        try:
            ...
            pass
    if st.session_state.prob_path and os.path.exists(st.session_state.prob_path):
        try:
            os.remove(st.session_state.prob_path)
        except:
            pass
